...
# ---------------- PREDICTION (FIXED & SAFE) ----------------
    ...
    if df is not None:
        ...
        with col1:
            city = st.selectbox("Select City", sorted(df["Location"].unique()))
        ...
        if b1.button("Predict"):
            ...
            # Filter city data
            city_df = df[df["Location"] == city_clean]

            if not city_df.empty:

                # Find nearest date
                city_df["date_diff"] = abs(city_df["Date"] - selected_date)
                nearest_row = city_df.sort_values("date_diff").iloc[0]
                ...
                <b>🌧️ Rainfall:</b> {round(row['Rainfall'], 2)} %

                <br><br>

                <b>📅 Nearest Available Date:</b> {row['Date'].date()} <br><br>

                <b>🌡️ Temperature:</b> {row['MaxTemp']} °C <br>
                <b>💧 Humidity:</b> {row['Humidity3pm']} % <br>
                <b>💨 Wind Speed:</b> {row['WindSpeed3pm']} km/h <br>
                <b>☁️ Cloud:</b> {row.get('Cloud3pm', 'N/A')} <br>
                <b>☀️ Sunshine:</b> {row.get('Sunshine', 'N/A')} <br>
