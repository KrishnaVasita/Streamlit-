...
# ---------------- PREDICTION (UPDATED ONLY RESULT ALIGNMENT) ----------------
    ...
    if b1.button("Predict"):
        ...
        if res["prediction"]==1:
            st.markdown(f'<div class="result-center success">🌧️ Rain Expected<br>Confidence: {round(res["probability"]*100,2)}%</div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="result-center fail">☀️ No Rain<br>Confidence: {round(res["probability"]*100,2)}%</div>', unsafe_allow_html=True)
